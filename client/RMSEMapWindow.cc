...
GLuint selectbufferRMSE[2048];

double  maxRMSE, minRMSE;


RMSEMapWindow::RMSEMapWindow(QWidget *parent) : Map3dGLWidget(parent, RMSEWINDOW, "RMSE Display",min_width, min_height)
{
...
RMSEMapWindow* RMSEMapWindow::RMSEMapWindowCreate(int _width, int _height, int _x, int _y)
{
    RMSEMapWindow* win = new RMSEMapWindow(masterWindow ? masterWindow->childrenFrame : NULL);
    win->positionWindow(_width, _height, _x, _y, default_width, default_height);
    win->show();
    return win;
}

void RMSEMapWindow::initializeGL()
{
    ...
    glSelectBuffer(2048, selectbufferRMSE);
    glLineStipple(1, 57795);
}

void RMSEMapWindow::paintGL()
{
    int loop = 0;
...
//    std::cout<< "maxRMSE in RMSE window is "<<maxRMSE<<std::endl;
//    std::cout<< "minRMSE in RMSE window is "<<minRMSE<<std::endl;
        ...
        for (loop2 = 0; loop2 < length; loop2++) {
            if (curmesh->shadingmodel == SHADE_GOURAUD) {
                ...
                for (loop3 = 0; loop3 < 3; loop3++) {
                    ...
                    if (cursurf->RMSEvals[index] == UNUSED_DATA)
                        break;
                }
                if (loop3 < 3)
                ...
                for (loop3 = 0; loop3 < 3; loop3++) {
                    index = curgeom->elements[loop2][loop3];
                    RMSEval = cursurf->RMSEvals[index];
                    ...
                    if (use_textures)
                    {
                        ...
                        glTexCoord1f(getContNormalizedValue(RMSEval, minRMSE, maxRMSE, curmesh->invert));
                    }
                ...
                mean /= 3;
                getContColor(mean, minRMSE, maxRMSE, curmesh->cmap, color, curmesh->invert);
                glColor4ub(color[0],color[1],color[2],color[3]);
...
void RMSEMapWindow::addMesh(Mesh_Info *curmesh)
{
    /* copy from meshes to this window's mesh list */
    meshes.push_back(curmesh);
    curmesh->RMSEpriv = this;
    recalcMinMax();
...
void RMSEMapWindow::recalcMinMax()
{
    xmin = ymin = zmin = FLT_MAX;
    xmax = ymax = zmax = -FLT_MAX;
    ...
    RMSEMapWindow* first_geom_window = GetRMSEMapWindow(0);
...
void RMSEMapWindow::mousePressEvent(QMouseEvent * event)
{
...
void RMSEMapWindow::mouseMoveEvent(QMouseEvent* event)
{
    HandleMouseMotion(event, (float)event->x() / (float)width(), (float)event->y() / (float)height());
}


void RMSEMapWindow::mouseReleaseEvent(QMouseEvent * event)
{
    HandleButtonRelease(event, (float)event->x() / (float)width(),(float)event->y() / (float)height());
}

void RMSEMapWindow::closeEvent(QCloseEvent * event)
{
    close();
}


void RMSEMapWindow::HandleButtonPress(QMouseEvent * event, float xn, float yn)
{
    int button = mouseButtonOverride(event);
...
void RMSEMapWindow::HandleMouseMotion(QMouseEvent * event, float xn, float yn)
{
    int button = mouseButtonOverride(event);
    int newModifiers = button == event->buttons() ? event->modifiers() : Qt::NoModifier;
    int x = int (width() * xn);
...
void RMSEMapWindow::HandleButtonRelease(QMouseEvent * event, float /*xn*/, float /*yn*/)
{
    int button = mouseButtonOverride(event);
...
void RMSEMapWindow::Transform(Mesh_Info * curmesh, float factor, bool compensateForRetinaDisplay)
{
   // std::cout<<"enter transform in RMSE window"<<std::endl;
...
void RMSEMapWindow::DrawBGImage()
{
...
int RMSEMapWindow::OpenMenu(QPoint point)
{
//    int all_node_num = 0;
...
//    action->setData(mark_all_RMSE_node); action->setCheckable(true); action->setChecked(all_node_num == 1);

//    action = submenu->addAction("RMSE time");
//    action->setData(mark_all_RMSE_value); action->setCheckable(true); action->setChecked(all_node_num == 2);
...
void RMSEMapWindow::MenuEvent(int menu)
{
    HandleMenu(menu);
}

void RMSEMapWindow::HandleMenu(int menu_data)
{
...
//        case mark_all_RMSE_node:
//            if (mesh->mark_all_RMSE_number == 1) {
//                // toggle the value
//                mesh->mark_all_RMSE_number = 0;
//            }
//            else {
//                mesh->mark_all_RMSE_number = 1;
//                mesh->qshowpnts = 1;
//            }
//            break;

//        case mark_all_RMSE_value:
//            if (mesh->mark_all_RMSE_number == 2) {
//                // toggle the value
//                mesh->mark_all_RMSE_number = 0;
//            }
//            else {
//                mesh->mark_all_RMSE_number = 2;
//                mesh->qshowpnts = 1;
//            }
//            break;
//        }
...
//    if (curmesh->mark_all_RMSE_number) {
...
//                number = curmesh->mark_all_RMSE_number;
//            }
...
//            case 2:
//                if (cursurf && cursurf->RMSEvals && cursurf->RMSEvals[loop] != UNUSED_DATA)
//                    renderString3f(pos[0], pos[1], pos[2], (int)small_font,
//                            QString::number(cursurf->RMSEvals[loop], 'g', 2), scale);
//                break;
