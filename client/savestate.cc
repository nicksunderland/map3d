...
void outputGlobalInfo(FILE* f, char* cont)
{
  // save -b
...
void outputMeshInfo(FILE* f, char* cont, Mesh_Info* mesh)
{
...
void SaveSettings(const char* filename, bool map3drc, bool windows)
{
  ...
  if (QFile::exists(filename) && !prompt_overwrite(NULL, filename))
  {
    return;
  }
