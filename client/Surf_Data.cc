  ...
  ***/
    ...
    if (newsurfnum == 0 || surfdata == NULL) {
        if ((surfdata = (Surf_Data *) calloc((size_t) 1, sizeof(Surf_Data))) == NULL) {
            fprintf(stderr, "*** AddaDataSurf: error getting 111 first memory\n");
            return (NULL);
        }
    ...
    if (numframes > 0 && numleads > 0) {
        if ((surfdata[newsurfnum].potvals = Alloc_fmatrix(numframes, numleads)) == NULL) {
            fprintf(stderr, "*** AddaDataSurf: error getting 222 first memory\n");
            return (NULL);
        }
    }
    else {
    ...
    /*** Now allocate some memory for the inverse in the surface. ***/

    if (numframes > 0 && numleads > 0) {

        if ((surfdata[newsurfnum].inversevals = Alloc_fmatrix(numframes, numleads)) == NULL) {
