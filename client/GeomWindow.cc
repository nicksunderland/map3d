...
GeomWindow::GeomWindow(QWidget* parent) : Map3dGLWidget(parent, GEOMWINDOW, "Geometry Display",min_width, min_height)
{
    ...
    showlocks = 1;
    l2norm = 0;
    gradientBackground = false;                      //Nick - initialise background to false then turn true if ABI GUI selected
    nodePickLock = false;                                //Nick - initialise to false to that it doesn't affect the running of Map3D
    cathTransLock = false;                               //Nick - initialise to false to that it doesn't affect the running of Map3D
    cathRotationLock = false;                           //Nick - initialise to false to that it doesn't affect the running of Map3D
    cathWinLock = false;                               //Nick
...
bool GeomWindow::Pick(int meshnum, int x, int y, bool del /*= false*/ )
{
    ...
    for (int i = 0; i < hits; i++) {
        ...
        if (map3d_info.pickmode == NEW_WINDOW_PICK_MODE || map3d_info.pickmode == REFRESH_WINDOW_PICK_MODE) {
            ...
            qDebug()<<"GeomWindow::Pick - processing hits and PickInfo creation";
            ...
            qDebug()<<"GeomWindow::Pick - processing hits. Pick node is: "<<pick->node;
            ...
            qDebug()<<"GeomWindow::Pick - start of Finding Nearest Electrode. int nummeshes is: "<<meshes.size();

            if ((nummeshes>1) && (pick->mesh->geom->surfnum >1))
            {
                qDebug()<<"GeomWindow::Pick - start of Finding Nearest Electrode, into IF statement. Recordingmesh = meshes["<<selection[3]<<"]";
                qDebug()<<"GeomWindow::Pick - start of Finding Nearest Electrode, into IF statement. Recordingmesh = meshes["<<selection[3]-1<<"]";
                recordingmesh=meshes[selection[3]-1];  //////////////////////// //Nick This is the part causing the crash with basket cath picking.
            ...
            if(map3d_info.GUIchoice == true){
                GeneratePick(pick);
                qDebug()<<"GeomWindow::Pick - generate Map3D pick";
            }else{
                emit sendPicktoGUI(pick);                                                                                //Nick - added this conditional statement to send the pick to the ABI GUI if we are in ABI mode.
                qDebug()<<"GeomWindow::Pick - send Pick to ABI GUI";
            }
