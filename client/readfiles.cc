...
#pragma mark Matlab Fiducials
        ...
        if (!inversevals.isempty())
        {
            ...
            //inversebuff = new float[numfileleads*numfileframes];
            if (inversebuff == NULL) {
                fprintf(stderr, "*** In ReadMatlabInverseFile I cannot get enough" " dynamic memory to buffer the data\n");
                return 0;
            }

            /*** Get the inverse buffer. ***/
            if (!inversevals.isempty()) inversevals.getnumericarray(inversebuff, inversevals.getnumelements());
            ...
            for (displaysurfnum = 0; displaysurfnum <= surfend - surfstart; displaysurfnum++) {
                ...
                for (framenum = framestart, modelframenum = 0; framenum <= frameend;
                    ...
                    for (nodenum = 0; nodenum < numsurfnodes; nodenum++) { // don't read higher nodes than your geom
                        ...
                        index = framenum * numfileleads +nodenum;
                        surf->inversevals[modelframenum][nodenum] = inversebuff[index] * potscale;
                    }
                }
                surfcount++;
            }
        ...
        for (framenum = framestart, modelframenum = 0; framenum <= frameend;
            ...
            for (nodenum = 0; nodenum < numsurfnodes; nodenum++) {
                if (map3d_geom[displaysurfnum].channels[nodenum] > -1 &&
                    ...
                    //std::cout<<'test test test'<<surf->potvals[modelframenum][nodenum]<<std::endl;//Shu Meng
                }
                else {
                    surf->potvals[modelframenum][nodenum] = UNUSED_DATA;
                    //std::cout<<surf->potvals[modelframenum][nodenum]+218<<std::endl;//Shu Meng
                }
            }
