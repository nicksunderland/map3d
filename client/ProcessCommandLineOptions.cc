...
{
    qDebug()<<"ProcessCommandLineOptions(Global_Input & g)";
   ...
   */
    ...
    if (length == 0) {
        ...
        //std::cout<<"call the function to show the first window"<<std::endl;



//  Nick - I have moved the Fd creation to int main()
//            filesDialogCreate();
//            filedialog->addRow(0);
...
qDebug()<<"ProcessCommandLineOptions(Global_Input & g) - EXIT.";
}
...
void FindAndReadData(Surf_Input * surf, Mesh_Info * mesh, int reload)
{
    ...
    case POT_ONLY:
        ...
        if (s == NULL) {
            printf("MAP3D WARNING: Unable to read data (potential POT_ONLY) file: %s\n", surf->potfilename);
            mesh->data = 0;
            return;
        }
        mesh->data = s;
    ...
    case MATLAB_FILE:
        ...
        if (s == NULL) {
            printf("MAP3D WARNING: Unable to read data (potential) file: %s\n", surf->potfilename);
            mesh->data = 0;
            return;
        }
        mesh->data = s;
...
void CopySurfToMesh(Surf_Input * s, Surf_Input* globalSurf, Mesh_Info * m)
{
    ...
    INITIALIZE_VALUE(transparent, transparent, -1, DEF_TRANSPARENT);
...
void GetDataFileInfo(std::string filename, int& numTimeSeries, std::vector<int>& numFramesPerSeries, std::vector<std::string>& timeSeriesLabels, MatlabIO::matlabarray*& outArray)
{
    ...
    qDebug()<<"ProcessCommandLineOptions - GetDataFileInfo() function";
    ...
    qDebug()<<"ProcessCommandLineOptions - GetDataFileInfo() function - EXIT.";
