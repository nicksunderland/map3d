    ...
    for (; loop < length; loop++) {
        ...
        case surface_render_banded:
            ...
            if (mesh->legendwin != 0)
                lpriv->update();
            break;
        case surface_render_transparent:
            MAP3D_MESH_LOCK_TOGGLE(map3d_info.lockgeneral, transparent);
...
bool GeomWindow::MenuGlobalOptions(int menu)
{
    ...
    case frame_lock:
        ...
        break;
    case transparent_point:

    {
        DrawTransparentPoints dialog;
        dialog.exec();
        break;
    }
    ...
    case scaling_dialog:
        ScaleDialog(0).exec();
        break;
    case rescaling_color_dialog:
    {
        RescaleColorMap dialog;
        dialog.exec();
        break;
    }
    case fid_dialog:
...
int GeomWindow::OpenMenu(QPoint point)
{
    ...
    submenu->addAction("Properties...")->setData(contour_dialog);
    ...
    submenu->addAction("Fixed Scale")->setData(rescaling_color_dialog);
