...
{
    qDebug()<<"FileDialogWidget::on_dataLineEdit_editingFinished ()";
    ...
    qDebug()<<"FileDialogWidget::on_dataLineEdit_editingFinished () - EXIT.";
}
...
{
    qDebug()<<"FileDialog::addRow(Mesh_Info* mesh)";
    ...
    qDebug()<<"FileDialog::addRow(Mesh_Info* mesh) - EXIT.";
}

void FileDialogWidget::updateRMS()
{
    qDebug()<<"FileDialogWidget::updateRMS()";
...
bool FileDialogWidget::updateFiles()
{
    ...
    // as far as I can tell, surf, ds, dstart and dend are 1-based to the user,winComboBox
    // but gs is not
...
{
    qDebug()<<"FileDialog ctor"; //Nick
    setupUi(this);
}

void FileDialog::on_applyButton_clicked()
{
    qDebug()<<"FileDialog::on_applyButton_clicked()";
    ...
    qDebug()<<"FileDialog::on_applyButton_clicked() - EXIT.";
}
...
void FileDialogWidget::on_endFrameSpinBox_editingFinished()
{

}

void FileDialogWidget::on_startFrameSpinBox_editingFinished()
{
...
//Nick - copied and then altered this function (see addition at the end) to input the data from the ABI file window.
void FileDialog::autoAddRow(Mesh_Info *mesh, QString geom, QString data, int index)
{
    bool empty_mesh = false;
    int surfnumber, window;
    if (!mesh) {
    ...
    //Nick
            widget->geomLineEdit->setText(geom);
            widget->on_geomLineEdit_editingFinished();
            widget->dataLineEdit->setText(data);
            widget->on_dataLineEdit_editingFinished();
            widget->winComboBox->setCurrentIndex(index);
}
