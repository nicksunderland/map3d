...
extern int fstep,cur,indexSpinBox_munualValue,sliderindex;
extern GetMatrixSlice* getmatrixslice;
QTimer *play_timer = new QTimer();
int Playindex;
...
bool GeomWindow::HandleFrameAdvances()
{
    ...
    if (map3d_info.lockframes) {
        ...
        //std::cout<<"key_pressed in geowindowhandle"<<key_pressed<<std::endl;
...

{
    ...
    if (map3d_info.lockframes) {
        Broadcast(MAP3D_SLIDER_FRAMES);
        //std::cout<<"Entering Broadcast(MAP3D_SLIDER_FRAMES) in lock mode "<<std::endl;
    }
    else {
...

{
    ...
    map3d_info.selected_group = (map3d_info.lockframes == LOCK_GROUP && meshes.size() > 0)
            ? meshes[0]->groupid : -1;
    if (indexSpinBox_munualValue!=cur+1){

        sliderindex=indexSpinBox_munualValue-1;
...
bool GeomWindow::PlayButtonControl()
...
bool GeomWindow::PauseButtonControl()
...
bool GeomWindow::AdvanceButtonControl()
{
    ...
    if (map3d_info.lockframes) {
        Broadcast(MAP3D_BUTTON_ADVANCE);
        //std::cout<<"Entering Broadcast(MAP3D_BUTTON_ADVANCE) in lock mode "<<std::endl;
    }
    else {
        std::cout<<"Warning: Frames Unlocked, use <- -> to control frames seperately "<<std::endl;
    }
...
bool GeomWindow::BackwardButtonControl()
{
    ...
    if (map3d_info.lockframes) {
        Broadcast(MAP3D_BUTTON_BACKWARD);
        //std::cout<<"Entering Broadcast(MAP3D_BUTTON_ADVANCE) in lock mode "<<std::endl;
    }
    else {
        std::cout<<"Warning: Frames Unlocked, use <- -> to control frames seperately "<<std::endl;
...
void GeomWindow::HandleKeyPress(QKeyEvent* event)
{
    ...
    for (; loop < length; loop++)
    {
        ...
        case 's':
            ...
            // transparent Shu Meng
        case 'b':
            menu_data = surface_render_transparent;
            HandleMenu(menu_data);
            loop = length; // this is so that the change happens n*n times (when
            // the general lock is on) - n times in this loop and n times
            // in HandleMenu
...
void GeomWindow::UpdateAndRedraw() 
{
    ...
    for (; loop < length; loop++) {
        ...
        // std::cout<<"UpdateTimestep in UpdateAndRedraw the framenum is  "<<mesh->data->framenum<<std::endl;
...
void GeomWindow::mousePressEvent(QMouseEvent * event)
{
    ...
    else if(cathRotationLock == true){
        Broadcast(MAP3D_MOUSE_BUTTON_PRESS, this, (QEvent *) event);
        //qDebug()<<"mousePressEvent: cathRotationLock = True";
    }

    else if(cathTransLock == true){
        Broadcast(MAP3D_MOUSE_BUTTON_PRESS, this, (QEvent *) event);
        //qDebug()<<"mousePressEvent: cathTransLock = True";
    }


    else if ((!map3d_info.lockrotate) || matchesModifiers(event->modifiers(), Qt::AltModifier, true) ||
             matchesModifiers(event->modifiers(), Qt::ControlModifier, true) || nodePickLock == true)                                //Nick - added this nodePickLock bool to let the node picking be controlled from the ABI GUI
    {
        qDebug()<<"mousePressEvent: lockrotate/alt/ctrl/nodePick - send to HandleButtonPress";
        HandleButtonPress(event, (float)event->x() / (float)width(), (float)event->y() / (float)height());

    }
    else {
        qDebug()<<"mousePressEvent: lockrotate/alt/ctrl/nodePick - send to Broadcast";
        Broadcast(MAP3D_MOUSE_BUTTON_PRESS, this, (QEvent *) event);
...
{
    qDebug()<<"GeomWindow::mouseReleaseEvent(QMouseEvent * event)";
...
void GeomWindow::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if(map3d_info.GUIchoice == false){                                                          //Nick

                        if(cathTransLock == true){
                            if(cathWinLock == true){
                                //qDebug()<<"mouseMoveEvent. cathTrans = True, cathWin = True. Send to HandleMouseMotion";
                                HandleMouseMotion(event, (float)event->x() / (float)width(), (float)event->y() / (float)height());

                            }else{
                                //qDebug()<<"mouseMoveEvent. cathTrans = True, cathWin = False. Send to Broadcast mouse motion";
                                Broadcast(MAP3D_MOUSE_MOTION, this, event);

                            }
                        }

                        if(cathRotationLock == true){
                            if(cathWinLock == true){
                                //qDebug()<<"mouseMoveEvent. cathRot = True, cathWin = True. Send to HandleMouseMotion";
                                HandleMouseMotion(event, (float)event->x() / (float)width(), (float)event->y() / (float)height());
                        ...
                        if(cathRotationLock == false && cathTransLock == false){

                            if ((!map3d_info.lockrotate) || matchesModifiers(event->modifiers(), Qt::AltModifier, true) ||
                                    matchesModifiers(event->modifiers(), Qt::ControlModifier, true)){

                                 //qDebug()<<"mouseMoveEvent. cathTrans && cathRot = False; noLockRotate/alt/ctrl. Send to HandleMouseMotion";

                                HandleMouseMotion(event, (float)event->x() / (float)width(), (float)event->y() / (float)height());

                            }else{
                                //qDebug()<<"mouseMoveEvent. cathTrans && cathRot = False; noLockRotate/alt/ctrl. Send to Broadcast Mouse Motion";
                                Broadcast(MAP3D_MOUSE_MOTION, this, event);
...
{
    qDebug()<<"HandleButtonPress function";
    ...
    for (loop = 0; loop < length; loop++) {
        ...
        else if (button == Qt::LeftButton && (matchesModifiers(newModifiers, Qt::ControlModifier, true) || nodePickLock == true) &&             //Nick - added this nodePickLock bool to let the node picking be controlled from the ABI GUI
                 ((!map3d_info.lockgeneral && (dominantsurf == loop || length == 1)) || map3d_info.lockgeneral)) {

            qDebug()<<"HandleButtonPress - LEFT MOUSE DOWN + CTRL = pick";
        ...
        // LEFT MOUSE DOWN = start rotate
            ...
            //qDebug()<<"HandleButtonPress function = left mouse button -->START ROTATE";
...
//Nick
void GeomWindow::setNodePickLock(bool nodePickStateFromGUI)
{
    nodePickLock = nodePickStateFromGUI;
}
void GeomWindow::setCathTransLock(bool cathTransLockFromGUI)
{
    cathTransLock = cathTransLockFromGUI;
}
void GeomWindow::setCathWinLock(bool cathWinLockFromGUI)
{
    cathWinLock = cathWinLockFromGUI;
}
void GeomWindow::setCathRotationLock(bool cathRotationLockFromGUI)
{
    cathRotationLock = cathRotationLockFromGUI;
}
//Nick
...
{
    //qDebug()<<"HandleMouseMotion function";
    ...
    if (length > 1 && !map3d_info.lockrotate)
        if (!map3d_info.lockgeneral) {
            ...
            //qDebug()<<"HandleMouseMotion function - loop is dominant surf.";
            ...
            //qDebug()<<"HandleMouseMotion function - loop is secondary surf.";
    ...
    for (; loop < length; loop++) {
        ...
        else if (button == Qt::LeftButton && (matchesModifiers(newModifiers, Qt::ShiftModifier, true)) || cathTransLock == true) {    //Nick - added this cathTransLock


            //qDebug()<<"HandleMouseMotion function - LEFT MOUSE DOWN + SHIFT --> translate.";
        ...
        // LEFT MOUSE DOWN + ABI GUI ROTATION CHECKBOX = rotate catheter   //Nick
        else if (button == Qt::LeftButton && cathRotationLock == true) {

             //qDebug()<<"HandleMouseMotion function - LEFT MOUSE DOWN + ABI rotate button --> rotate";
        ...
        // LEFT MOUSE DOWN = rotate
            ...
            //qDebug()<<"HandleMouseMotion function - LEFT MOUSE DOWN -->rotate";
...
//void GeomWindow::connectButtonToExecuteSignal(QAbstractButton* button)
//{
